...
Для оплаты по криптограмме, сформированной скриптом [Checkout](#skript-checkout), [Apple Pay](#appe-pay) или [Google Pay](#google-pay) необходимо выполнить вызов метода по одному из следующих адресов:

* `POST https://api.cloudpayments.ru/payments/cards/charge` — для [одностадийного платежа](#shemy-provideniya-platezha)  
* `POSThttps://api.cloudpayments.ru/payments/cards/auth`  — для [двухстадийного](#shemy-provideniya-platezha)  
...
JsonData | Json | Необязательный | Любые другие данные, которые будут связаны с транзакцией, в том числе инструкции для [создания подписки](#rekurrentnye-platezhi) или формирования [онлайн-чека](#format-peredachi-dannyh).Мы зарезервировали названия следующих параметров и отображаем их содержимое в реестре операций, выгружаемом в Личном Кабинете: name, firstName, middleName, lastName, nick, phone, address, comment, birthDate.
...
* `https://api.cloudpayments.ru/payments/tokens/charge` — для [одностадийного платежа](#shemy-provideniya-platezha)  
или  
* `https://api.cloudpayments.ru/payments/tokens/auth` — для [двухстадийного](#shemy-provideniya-platezha).
...
Email | String | Необязательный |  	E-mail плательщика на который будет отправлена квитанция об оплате
JsonData | Json | Необязательный | Любые другие данные, которые будут связаны с транзакцией, в том числе инструкции для [создания подписки](#rekurrentnye-platezhi) или формирования [онлайн-чека](#format-peredachi-dannyh).
...
Для платежей, проведенных по [двухстадийной схеме](#shemy-provideniya-platezha), необходимо подтверждение оплаты, которое можно выполнить через личный кабинет либо через вызов метода API.
...
JsonData | Json | Необязательный | Любые другие данные, которые будут связаны с транзакцией, в том числе инструкции для формирования [онлайн-чека](#format-peredachi-dannyh).
