...
Currency | String |	Обязательный |	Валюта: RUB/USD/EUR/GBP из параметров виджета (см. [справочник](#spisok-valyut))
...
Status |	String |	Обязательный |	Статус платежа в случае успешного завершения: Completed — для [одностадийнх платежей](#shemy-provideniya-platezha), Authorized — для [двухстадийных](#shemy-provideniya-platezha)
OperationType |	String |	Обязательный |	Тип операции: Payment/Refund/CardPayout (см. [справочник](#tipy-operatsiy))
...
Выполняется после подтверждения платежа, проведенного по [двухстадийной](#shemy-provideniya-platezha) схеме.
...
Выполняется в случае, если платеж был возвращен (полностью или частично) по вашей инициативе через [API](#api) или личный кабинет.
...
Currency |	String |	Обязательный |	Валюта: RUB/USD/EUR/GBP из параметров виджета. (см. [справочник](#spisok-valyut))
RequireConfirmation |	Bool |	Обязательный |	Если значение `true` — платеж будет выполнен по [двустадийной схеме](#shemy-provideniya-platezha)
...
Type |	Строка |	Обязательный |	Признак расчета, см. [справочник](#tipy-online-chekov)
...
Выполняется в случае, если платеж был отменён по вашей инициативе через [API](#api) или личный кабинет.
